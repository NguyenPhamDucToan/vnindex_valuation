...
from valuation.ratios import (
    gross_margin, net_margin, operating_margin, roe, roa, roic,
...
def load_company_info(ticker: str) -> tuple[str, str, str]:
    ...
    with get_session() as s:
        row = s.execute(select(Company).where(Company.ticker == ticker)).scalar_one_or_none()
        if row:
            return (row.name or ticker), (row.exchange or "HOSE"), (row.sector or "—")
    return ticker, "HOSE", "—"
...
def _render_index_ticker_bar():
    ...
    for _col, (_sym, _d) in zip(_idx_cols, _idx_data.items()):
        ...
        _fig_i.add_annotation(x=0.5, y=1.58, xref="paper", yref="paper",
            ...
            showarrow=False, font=dict(size=11, color="#6b7280"), xanchor="center")
...
if view == "Phân tích Cổ phiếu":
    ...
    # ── Styled stock header ────────────────────────────────────
    _co_name, _co_exch, _co_sect = load_company_info(ticker)
    ...
    def _price_chart_frag(ticker):
        ...
        ttm           = load_ttm(ticker)
        _, _, _co_sect = load_company_info(ticker)
        valuations    = get_all_valuations(ticker)
        ...
        with col_chart:
            ...
            # For 2Y/3Y: downsample daily → weekly to cut chart payload ~5×
            if _n_days > 252 and len(df1y) > 0:
                _widx = pd.to_datetime(df1y["date"])
                df1y = (
                    df1y.set_index(_widx)
                    .resample("W-FRI")
                    .agg(
                        open=("open", "first"), high=("high", "max"),
                        low=("low", "min"),   close=("close", "last"),
                        volume=("volume", "sum"),
                        open_vnd=("open_vnd", "first"), high_vnd=("high_vnd", "max"),
                        low_vnd=("low_vnd", "min"),    close_vnd=("close_vnd", "last"),
                        ma10=("ma10", "last"),          ma50=("ma50", "last"),
                    )
                    .dropna(subset=["open"])
                ...
                df1y["dlabel"] = df1y["date"].dt.strftime("%Y-%m-%d")
    ...
    _price_chart_frag(ticker)

    st.divider()

    @st.fragment
    def _financials_frag(ticker):
        fin_q          = load_financials_q(ticker)
        _prices_df_f   = load_prices(ticker)
        current_price  = float(_prices_df_f["close"].iloc[-1]) * 1000
        ttm            = load_ttm(ticker)
        valuations     = get_all_valuations(ticker)
        _, _, _co_sect = load_company_info(ticker)
        ...
        if not fin_q.empty and len(fin_q) >= 4:
            ...
            fig_mg.add_hline(y=0, line_dash="dot", line_color="#64748b", opacity=0.4)
            ...
            # st.tabs() runs all tabs' bodies on every rerun even when only one is visible
            # — segmented_control + if/elif runs only the active tab's (expensive) chart code.
            _fin_tab_sel = st.segmented_control(
                "Báo cáo tài chính",
                ["Kết quả KD", "Cân đối KT", "Dòng tiền & Tỷ số"],
                default="Kết quả KD",
                key="fin_tab_sel",
                label_visibility="collapsed",
            ) or "Kết quả KD"

            if _fin_tab_sel == "Kết quả KD":
                c1, c2, c3 = st.columns(3)
                ...
                if _co_sect == "Ngân hàng":
                    ...
                    fig_b2.add_trace(go.Scatter(x=labels, y=dep_yoy, name="Tăng trưởng YoY %",
                        ...
                        secondary_y=True)
                    fig_b2.add_hline(y=0, line_dash="dot", line_color="#64748b",
                                     opacity=0.4, secondary_y=True)
                ...
                with r2c1:
                    ...
                    fig4.add_hline(y=0, line_dash="dot", line_color="#64748b", opacity=0.4, secondary_y=True)
                    fig4.update_layout(
                ...
                if _is_bank:
                    ...
                    with r3c1:
                        ...
                        fig7.add_hline(y=0, line_dash="dot", line_color="#64748b", opacity=0.4)
                    ...
                    with r3c2:
                        ...
                        fig8.add_hline(y=0, line_dash="dot", line_color="#64748b", opacity=0.4, secondary_y=False)
                    ...
                    with r3c3:
                        ...
                        fig9.add_hline(y=0, line_dash="dot", line_color="#64748b", opacity=0.4, secondary_y=False)
                ...
                if _is_bank:
                    ...
                    with r4c2:
                        ...
                        fig11b.add_trace(go.Bar(x=labels, y=other_fund, name="Nợ phải trả khác",
                            marker_color="#475569", hovertemplate="%{y:,.0f} tỷ<extra></extra>"))
                    ...
                    with r4c3:
                        ...
                        fig12.add_hline(y=1, line_dash="dot", line_color="#64748b", opacity=0.5)
                ...
                if _is_bank:
                    ...
                    with r5c1:
                        ...
                        fig13b.add_trace(go.Scatter(x=labels, y=loans_yoy, name="Tăng trưởng YoY %",
                            ...
                            marker=dict(size=5), hovertemplate="%{y:.1f}%<extra></extra>"),
                            secondary_y=True)
                        fig13b.add_hline(y=0, line_dash="dot", line_color="#64748b",
                                         opacity=0.4, secondary_y=True)
                    ...
                    with r5c2:
                        ...
                        fig14b.add_hline(y=0, line_dash="dot", line_color="#64748b",
                    ...
                    with r5c1:
                        ...
                        fig13.add_hline(y=0, line_dash="dot", line_color="#64748b", opacity=0.4)
                        fig13.update_layout(
                        ...
                        if _co_sect == "Chứng khoán":
                            ...
                            if any(v is not None for v in inv_pct):
                                fig14.add_trace(go.Scatter(x=labels, y=inv_pct, name="Tồn kho/Tổng TS %",
                                    mode="lines", line=dict(color="#c00000", width=2),
                                    hovertemplate="%{y:.1f}%<extra></extra>"), secondary_y=True)
                            fig14.add_hline(y=0, line_dash="dot", line_color="#64748b", opacity=0.4, secondary_y=False)
            ...
            elif _fin_tab_sel == "Dòng tiền & Tỷ số":
                ...
                with r6c1:
                    ...
                    fig16.add_hline(y=0, line_dash="dot", line_color="#64748b", opacity=0.4, secondary_y=False)
                    fig16.update_layout(
                ...
                with r7c1:
                    ...
                    fig19.add_hline(y=0, line_dash="dot", line_color="#64748b", opacity=0.4,
                                    secondary_y=False)
                    fig19.update_layout(
            ...
            if _avg_roic is not None and _wacc_val is not None:
                ...
                if len(_roic_by_year) >= 2:
                    ...
                    _fig_rw.add_trace(go.Scatter(
                        ...
                        text=[f"  {v:.1f}%" if i < len(_roic_vals)-1 else f"{v:.1f}%" for i, v in enumerate(_roic_vals)],
                        textposition=["top right"] * (len(_roic_vals)-1) + ["top left"],
...
# ═══════════════════════════════════════════════════════════════
    ...
    """, unsafe_allow_html=True)
            ...
            fig_qs.add_vline(x=0, line_dash="dot", line_color="#64748b", opacity=0.4)
            fig_qs.add_hline(y=50, line_dash="dot", line_color="#64748b", opacity=0.4)
    ...
    if _mo_sel == "📊 Thị trường":
        ...
        with col_idx:
            ...
            if not vnidx.empty:
                ...
                _first_vi = float(_vi.iloc[0]["close"])
                _yr_chg_vi = (_last_vi - _first_vi) / _first_vi * 100 if _first_vi else 0
                _yr_cc_vi  = "#22c55e" if _yr_chg_vi >= 0 else "#ef4444"
        ...
        with sec_col:
            ...
            _fig_sp = go.Figure(go.Bar(
                ...
                hovertemplate="%{y}: %{x:+.2f}%<extra></extra>"))
            _fig_sp.add_vline(x=0, line_dash="dot", line_color="#64748b", opacity=0.5)
